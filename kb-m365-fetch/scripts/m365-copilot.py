...
def build_default_prompt() -> str:
    ...
    return (
        f"Search Teams CHANNEL messages (not emails, not 1:1 chats) from the LAST 24 HOURS "
        f"for everything related to {project_name} or any technical project topic.\n"
        f"{keyword_line}"
        "I already get emails through another channel — do NOT include emails. Only "
        "Teams channel messages.\n\n"
        "Do NOT write an executive summary. List ALL facts with maximum detail:\n"
        "- Full names of people and their roles/companies\n"
        "- Exact model names, versions, endpoints, URLs, namespaces, repos, pods\n"
        "- Commands, file paths, YAML names, StorageClasses, config keys\n"
        "- Jira statuses, sprint changes, assignments\n"
        "- Blockers, errors, timeouts, workarounds and who reported/resolved them\n"
        "- Decisions made AND pending decisions with who is involved\n"
        "- Dates and times when mentioned\n"
        "- Name of the Teams channel where each topic was discussed\n\n"
        "Structure the response by technical topic, not by channel. Include EVERYTHING, "
        "even what seems minor — another agent needs exact data to "
